...
class Pix2PixModel(BaseModel):
    ...
    def backward_D(self):
        ...
        if self.use_condition == 1:
            ...
            fake_AB = self.fake_B
        self.loss_D_real = self.netD(self.real_A.detach())
        
        self.loss_D_fake = self.netD(fake_AB.detach())
        
        self.loss_D = torch.mean(nn.ReLU(inplace=True)(1.0 - self.loss_D_real)) + torch.mean(nn.ReLU(inplace=True)(1 + self.loss_D_fake))
